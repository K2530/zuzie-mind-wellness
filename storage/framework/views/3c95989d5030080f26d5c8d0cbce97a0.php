<header id="main-header" class="sticky top-0 z-40 border-b border-reseda/10 bg-white/92 backdrop-blur transition-all duration-300">
  <div id="header-container" class="container-header flex min-h-20 items-center justify-between gap-3 transition-all duration-300">
    ...
    <nav class="hidden items-center gap-8 text-[13px] font-semibold text-ink lg:flex" aria-label="Main navigation">
      <?php $__currentLoopData = $navItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <a class="whitespace-nowrap transition hover:text-reseda" href="<?php echo e($item['href']); ?>" data-i18n="<?php echo e($item['key']); ?>"></a>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </nav>

    <div class="hidden items-center gap-2 lg:flex">
      <?php if(session('logged_in')): ?>
        <a href="<?php echo e(route('booking')); ?>" class="btn-consult min-h-11 px-3 py-2" data-i18n="bookConsult"></a>
        <a href="https://line.me/R/ti/p/@zuziewellness" class="btn-line min-h-11 px-3 py-2" rel="noopener" data-i18n="line"></a>
        <a href="<?php echo e(route('cart')); ?>" class="text-ink hover:text-reseda transition relative ml-2" aria-label="Cart">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
          <?php if(session('cart')): ?>
            <span class="absolute -top-1 -right-1 bg-[#e35641] text-white text-[9px] font-bold w-4 h-4 flex items-center justify-center rounded-full"><?php echo e(count(session('cart'))); ?></span>
          <?php endif; ?>
        </a>
        <div class="relative ml-2">
            <button type="button" id="user-menu-btn" class="relative flex items-center justify-center w-9 h-9 rounded-full bg-[#1c1d1f] text-white font-bold hover:bg-black transition focus:outline-none" aria-expanded="false">
              <?php echo e(session('user.initial')); ?>
            ...
            <div id="user-menu" class="absolute right-0 mt-3 w-[260px] bg-white rounded-lg shadow-[0_4px_16px_rgba(0,0,0,0.1)] py-2 border border-gray-100 hidden z-50">
              <div class="px-4 py-3 flex items-center gap-3">
                ...
                <div class="overflow-hidden">
                  <p class="text-[15px] font-bold text-ink truncate"><?php echo e(session('user.name')); ?></p>
                  <p class="text-[13px] text-ink/70 truncate"><?php echo e(session('user.email')); ?></p>
                </div>
              </div>
              <hr class="border-gray-100 my-1">
              <a href="<?php echo e(route('my.courses')); ?>" class="block px-4 py-2 text-[15px] text-ink hover:text-reseda transition" data-i18n="myLearning">คอร์สเรียนของฉัน</a>
              <a href="<?php echo e(route('cart')); ?>" class="block px-4 py-2 text-[15px] text-ink hover:text-reseda transition flex justify-between items-center">
                <span data-i18n="myCart">ตะกร้าสินค้า</span>
                <?php if(session('cart')): ?>
                  <span class="bg-[#e35641] text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full"><?php echo e(count(session('cart'))); ?></span>
                <?php endif; ?>
              </a>
              ...
              <hr class="border-gray-100 my-1">
              <a href="<?php echo e(route('logout')); ?>" class="block px-4 py-2 text-[15px] text-ink hover:text-reseda transition" data-i18n="logout">ออกจากระบบ</a>
            </div>
        </div>
      <?php else: ?>
        <div>
        ...
        <a href="<?php echo e(route('booking')); ?>" class="btn-consult min-h-11 px-3 py-2" data-i18n="bookConsult"></a>
        <a href="https://line.me/R/ti/p/@zuziewellness" class="btn-line min-h-11 px-3 py-2" rel="noopener" data-i18n="line"></a>
        <a href="<?php echo e(route('login')); ?>" class="btn-secondary border-reseda text-reseda hover:bg-reseda hover:text-white min-h-11 px-4 py-2" data-i18n="navLogin">เข้าสู่ระบบ</a>
      <?php endif; ?>
    </div>
  ...
  <div id="mobile-menu" class="hidden border-t border-reseda/10 bg-white lg:hidden" data-mobile-menu>
    <div class="container-header grid gap-2 py-4">
      <?php $__currentLoopData = $navItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <a class="rounded-md px-3 py-3 text-base font-semibold hover:bg-almond" href="<?php echo e($item['href']); ?>" data-i18n="<?php echo e($item['key']); ?>"></a>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      <div class="mt-2 grid grid-cols-2 gap-2">
        <?php if(session('logged_in')): ?>
          <div class="col-span-2 px-3 py-4 border border-reseda/10 rounded-lg mb-2">
            <div class="flex items-center gap-3 mb-4">
              ...
              <div class="overflow-hidden">
                <p class="text-sm font-bold text-ink truncate"><?php echo e(session('user.name')); ?></p>
                <p class="text-[12px] text-ink/70 truncate"><?php echo e(session('user.email')); ?></p>
              </div>
            </div>
            <a href="<?php echo e(route('my.courses')); ?>" class="block py-2 text-sm text-ink hover:text-reseda transition" data-i18n="myLearning">คอร์สเรียนของฉัน</a>
            <a href="<?php echo e(route('cart')); ?>" class="block py-2 text-sm text-ink hover:text-reseda transition flex justify-between items-center">
              <span data-i18n="myCart">ตะกร้าสินค้า</span>
              <?php if(session('cart')): ?>
                <span class="bg-[#e35641] text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full"><?php echo e(count(session('cart'))); ?></span>
              <?php endif; ?>
            ...
            <hr class="border-reseda/10 my-2">
            <a href="<?php echo e(route('logout')); ?>" class="block py-2 text-sm text-ink hover:text-reseda transition" data-i18n="logout">ออกจากระบบ</a>
          </div>
          <a href="<?php echo e(route('booking')); ?>" class="btn-consult col-span-2" data-i18n="bookConsult"></a>
        ...
        <?php else: ?>
          ...
          <a href="<?php echo e(route('login')); ?>" class="btn-secondary border-reseda text-reseda col-span-2" data-i18n="navLogin">เข้าสู่ระบบ</a>
          <a href="<?php echo e(route('booking')); ?>" class="btn-consult col-span-2" data-i18n="bookConsult"></a>
          <a href="https://line.me/R/ti/p/@zuziewellness" class="btn-line col-span-2" rel="noopener" data-i18n="line"></a>
        <?php endif; ?>
      </div>
...
<?php /**PATH C:\Users\khach\Documents\Codex\2026-05-29\frongkung1-zuzie-mind-wellness-git-https\resources\views/partials/header.blade.php ENDPATH**/ ?>