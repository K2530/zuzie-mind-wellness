...
<head>
  ...
  <title>Zuzie Mind Wellness</title>
  ...
  <main id="home">
    <section class="relative overflow-hidden bg-almond">
      <div class="absolute inset-0">
        <img src="<?= e(asset('images/hero-woman-tea.png')) ?>" alt="Woman holding tea in a warm wellness room" class="h-full w-full object-cover object-[68%_50%]">
      </div>
      ...
      <div class="container-soft relative grid min-h-[760px] items-center pb-36 pt-16 sm:min-h-[800px] lg:min-h-[780px]">
        <div class="max-w-2xl pt-8 drop-shadow-[0_2px_18px_rgba(255,252,248,0.65)]">
          ...
          <div class="mt-8 grid gap-3 sm:grid-cols-3">
            ...
            <a href="#videos" class="btn-secondary" data-i18n="heroVideoCta"></a>
          </div>
        </div>
      </div>
    </section>
      ...
      <div class="mx-auto grid max-w-6xl gap-3 rounded-[24px] border border-reseda/10 bg-white/94 px-4 py-5 shadow-[0_18px_45px_rgba(83,76,65,0.10)] sm:grid-cols-2 lg:grid-cols-6 lg:px-7 lg:py-6">
        <?php foreach ($trustItems as $item): ?>
          <div class="grid justify-items-center gap-2 rounded-md px-2 py-1 text-center">
            <span class="icon-shell" data-icon="<?= e($item['icon']) ?>" aria-hidden="true"></span>
            <p class="text-[13px] font-medium leading-5 text-ink" data-i18n="<?= e($item['key']) ?>"></p>
          </div>
        <?php endforeach; ?>
      </div>
    </section>
    ...
    <section id="assessment" class="container-soft grid gap-8 pb-16 pt-20 lg:grid-cols-[260px_1fr] lg:items-start">
      ...
      <div class="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
        <?php foreach ($assessments as $card): ?>
          <article class="soft-card overflow-hidden">
            <img src="<?= e(asset('images/hero-woman-tea.png')) ?>" alt="" class="h-36 w-full object-cover <?= e($card['pos']) ?>">
            <div class="p-5">
              <p class="text-sm text-ink/65" data-i18n="assessmentLabel"></p>
              <h3 class="mt-1 text-xl font-bold" data-i18n="<?= e($card['key']) ?>"></h3>
              <p class="mt-3 min-h-14 text-sm leading-6 text-ink/70" data-i18n="<?= e($card['desc']) ?>"></p>
    ...
    <section id="videos" class="border-y border-reseda/10 bg-white py-14">
      <div class="container-soft grid gap-8 lg:grid-cols-[260px_1fr]">
        <div>
          ...
          <a href="#videos" class="btn-primary mt-7" data-i18n="viewAllVideos"></a>
        </div>
        <div class="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          <?php foreach ($videos as $index => $video): ?>
            <article class="group">
              <div class="relative overflow-hidden rounded-lg bg-reseda">
                <img src="<?= e(asset('images/hero-woman-tea.png')) ?>" alt="" class="h-36 w-full object-cover opacity-75 transition group-hover:scale-105 <?= $index % 2 ? 'object-[72%_48%]' : 'object-[62%_44%]' ?>">
                <div class="absolute inset-0 bg-gradient-to-t from-ink/68 to-transparent"></div>
                <span class="absolute bottom-3 right-3 rounded bg-ink/80 px-2 py-1 text-xs font-bold text-white"><?= e($video['time']) ?></span>
              </div>
              <h3 class="mt-3 line-clamp-2 text-sm font-bold leading-6"><?= e($video['title']) ?></h3>
              <p class="mt-1 text-sm text-ink/55"><?= e($video['source']) ?></p>
            </article>
          <?php endforeach; ?>
        </div>
      </div>
      ...
      <div class="container-soft relative">
        ...
        <div class="mt-8 flex flex-wrap gap-3">
          <?php foreach ($hashtags as $tag): ?>
            <a href="#articles" class="pill"><?= e($tag) ?></a>
          <?php endforeach; ?>
