<x-layouts.app :nav-items="$navItems" title="Zuzie Mind Wellness - Booking Successful">
  <main class="bg-milk min-h-screen pb-20 pt-20 flex items-center justify-center">
    <div class="container-soft max-w-2xl text-center">
      ...
      <h1 class="text-3xl sm:text-4xl font-extrabold text-ink mb-4">จองคิวสำเร็จ!</h1>
      <p class="text-ink/80 mb-8 text-lg">ขอบคุณที่ไว้วางใจให้เราดูแล เราได้ส่งรายละเอียดการจองไปยังอีเมลของคุณเรียบร้อยแล้ว</p>

      @if(session('booking'))
        <div class="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-reseda/10 max-w-md mx-auto mb-10 text-left">
          <h3 class="font-bold text-ink text-lg mb-4 text-center border-b border-gray-100 pb-4">รายละเอียดการจอง</h3>
          ...
          <div class="space-y-4">
            <div class="flex items-start gap-4">
              ...
              <div>
                <p class="text-xs font-bold text-ink/50 uppercase tracking-wider mb-1">บริการ</p>
                <p class="font-bold text-ink">{{ session('booking')['service'] ?? 'คุยกับนักจิตวิทยา' }}</p>
              </div>
            </div>
            ...
            <div class="flex items-start gap-4">
              ...
              <div>
                <p class="text-xs font-bold text-ink/50 uppercase tracking-wider mb-1">วันที่และเวลา</p>
                <p class="font-bold text-ink">{{ session('booking')['date'] ?? '15 พฤษภาคม 2024' }}</p>
                <p class="text-ink/80 text-sm mt-0.5">เวลา {{ session('booking')['time'] ?? '18:00' }} น.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="flex flex-col sm:flex-row gap-4 justify-center">
        <a href="{{ route('home') }}" class="bg-[#536442] text-white hover:bg-[#425034] transition rounded-full px-10 py-3.5 text-[15px] font-bold w-full sm:w-auto text-center shadow-md">
          กลับหน้าหลัก
        </a>
      </div>
