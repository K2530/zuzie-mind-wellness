      ...
      <section class="card">
        <div class="flex items-center justify-between border-b border-reseda/10 pb-5">
          <div>
            ...
            <p class="mt-2 text-sm text-ink/70">ผลการประเมินสุขภาพจิต</p>
          </div>
        ...
        <div class="py-10 text-center">
          @php
              if ($level === 'high') {
                  ...
                  $desc = 'ยอดเยี่ยมมากครับ! สุขภาพจิตของท่านอยู่ในเกณฑ์ที่ดีกว่าคนทั่วไป แสดงให้เห็นถึงการดูแลจิตใจตัวเองได้เป็นอย่างดี ขอให้มีความสุขสดใสแบบนี้ทุกวันนะครับ';
                  ...
                  $desc = 'สุขภาพจิตของท่านอยู่ในเกณฑ์ปกติเท่ากับคนทั่วไปครับ ถือว่าท่านมีความสามารถในการรับมือกับเรื่องราวต่างๆ ในชีวิตประจำวันได้ดีทีเดียว';
              } else {
                  $color = '#d97706'; // Orange
                  $bg_color = '#d9770615';
                  $percent = ($score_total / 60) * 100;
                  ...
                  $desc = 'ช่วงนี้ท่านอาจมีเรื่องกังวลใจหรือความเครียดสะสม หากรู้สึกไม่สบายใจ ท่านสามารถขอรับคำปรึกษาจากลูกหลาน เพื่อนสนิท หรือเจ้าหน้าที่สาธารณสุขใกล้บ้านได้เลยนะครับ';
              }
          @endphp
          ...
          <div class="relative mx-auto flex size-48 items-center justify-center rounded-full" style="background: conic-gradient({{ $color }} {{ $percent }}%, #e2e8f0 0)">
            <div class="absolute flex size-[172px] flex-col items-center justify-center rounded-full bg-milk shadow-inner">
              ...
              <span class="text-sm font-semibold text-ink/40 mt-1">เต็ม 60</span>
            </div>
          </div>
          ...
          <div class="mt-8 rounded-2xl p-6 border" style="background-color: {{ $bg_color }}; border-color: {{ $color }}30">
            <div class="flex flex-col items-center gap-3">
              ...
              <p class="text-ink/80 max-w-lg mt-2 leading-relaxed text-lg">{{ $desc }}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div class="space-y-3">
          @foreach($assessment['questions'] as $index => $q)
            ...
            <div class="flex items-start gap-4 rounded-xl border border-reseda/10 bg-milk/30 p-4 hover:bg-milk/60 transition-colors">
              <span class="flex size-8 shrink-0 items-center justify-center rounded-full bg-reseda/15 text-lg font-bold text-ink/70">
                {{ $index + 1 }}
              </span>
              <div class="flex-1 pt-1 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                <p class="text-ink/80 text-base font-medium">{{ is_array($q) ? $q['text'] : $q }}</p>
                <div class="flex items-center gap-2 shrink-0">
                  <span class="rounded-full px-4 py-1 text-sm font-semibold bg-white border border-ink/10 shadow-sm text-ink/70">
                    {{ $text_ans }}
                  </span>
                  @if($is_negative)
