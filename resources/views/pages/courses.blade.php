  ...
  <main class="bg-milk pb-20">
    <section class="py-16 sm:py-20">
      <div class="container-soft text-center">
        <h1 class="text-3xl font-extrabold text-ink sm:text-4xl">คอร์สออนไลน์</h1>
        <p class="mt-4 text-base text-ink/70">เรียนรู้ เข้าใจ และเยียวยาใจตัวเองได้จากที่บ้าน</p>
      </div>
      ...
      <div class="container-soft mt-12">
            ...
            @if ($index === 0)
              ...
              <div class="rounded-2xl p-6 flex flex-col justify-between shadow-[0_18px_45px_rgba(83,76,65,0.08)] bg-gradient-to-b from-[#404c3e] to-[#2a3429] text-white min-h-[400px] relative overflow-hidden">
                ...
                <div class="mt-12">
                  ...
                  <p class="text-sm text-white/80 leading-relaxed">{{ $course['desc'] }}</p>
                </div>
                <div class="mt-8">
                  <div class="flex items-end gap-2 mb-4">
                    <span class="text-xl font-bold">{{ $course['price'] }} บาท</span>
                    @if ($course['old_price'])
                      <span class="text-sm line-through text-white/60">{{ $course['old_price'] }} บาท</span>
                    @endif
                  </div>
                  <a href="{{ route('course.detail', 1) }}" class="block w-full text-center bg-white text-ink font-bold py-3 rounded-lg hover:bg-milk transition">
                    ดูรายละเอียด
                  </a>
                </div>
              </div>
            @else
              ...
              <a href="{{ route('course.detail', $index + 1) }}" class="rounded-2xl flex flex-col justify-between shadow-[0_18px_45px_rgba(83,76,65,0.08)] relative overflow-hidden min-h-[400px] group cursor-pointer">
                ...
                <div class="relative z-10 p-6 flex flex-col justify-end h-full text-white">
                  ...
                  @endif
                  <div>
                    <h2 class="text-xl font-bold mb-2 group-hover:text-reseda transition-colors">{{ $course['title'] }}</h2>
                    <p class="text-sm text-white/80 leading-relaxed mb-4">{{ $course['desc'] }}</p>
                    <div class="flex items-center gap-2">
                      <span class="text-lg font-bold">{{ $course['price'] }} บาท</span>
                      @if ($course['old_price'])
                        <span class="text-xs line-through text-white/60">{{ $course['old_price'] }} บาท</span>
                      @endif
                    </div>
                  </div>
                </div>
        ...
        <div class="mt-12 text-center">
          <a href="#" class="btn-primary inline-block px-12">ดูคอร์สทั้งหมด</a>
        </div>
      </div>
    </section>
  </main>
